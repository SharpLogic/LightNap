...
describe('RxJsHelpers', () => {
    describe('distinctUntilJsonChanged', () => {
        it('should emit when JSON representation changes', async () => {
            const values: any[] = [];
            const source$ = of({ id: 1, name: 'John' }, { id: 1, name: 'John' }, // Duplicate - should be filtered
            { id: 2, name: 'Jane' });
        ...
        it('should detect changes in nested objects', async () => {
            const values: any[] = [];
            const source$ = of({ user: { id: 1, profile: { name: 'John' } } }, { user: { id: 1, profile: { name: 'John' } } }, // Duplicate
